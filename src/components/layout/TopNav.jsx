...
const TopNav = ({ mode, setMode, aiVision, setAiVision }) => {
    ...
    // 3. Helper to switch modes and routes
    const handleModeSwitch = (newMode) => {
        setMode(newMode);
        if (newMode === 'developer') {
        navigate('/developing/sessions');
        } else {
    ...
    return (
        ...
        {user && (
            ...
            <button
                onClick={() => handleModeSwitch('learner')} // 4. Update Click
            ...
            <button
                onClick={() => handleModeSwitch('developer')} // 5. Update Click
                className={`px-6 py-1 rounded-full text-sm font-medium transition-all ${
        ...
        <div className="flex items-center gap-5">
            ...
            <>
                ...
                className={`flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-md border transition-all ${
                    aiVision ? 'border-green-500 text-green-500 bg-green-500/10' : 'border-gray-600 text-gray-400'
                }`}
                >
                AI Vision
                </button>
                <div className="flex items-center gap-3 border-l border-gray-700 pl-5">
                <button onClick={logoutUser} className="text-gray-500 hover:text-red-400">
                    <ArrowRightOnRectangleIcon className="w-6 h-6" />
                </button>
                </div>
            </>
            ) : (
            <Link to="/login" className="text-sm bg-blue-600 px-4 py-1.5 rounded-md text-white">Login</Link>
            )}
        </div>
        </nav>
    )
